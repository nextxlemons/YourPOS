...
```
POS/
├── POS/                # Project settings & root urls.py
├── home/                # Main app
│   ├── models.py        # TableInfo, MenuCategory, MenuItem, MenuVariant, Order, OrderItem, Bill
│   ├── views.py          # Page views + JSON endpoints for AJAX
│   ├── urls.py            # App-level routes
│   └── templates/
│       ├── additems.html
│       ├── base.html
│       ├── createorders.html # Per-table ordering & billing screen
│       ├── edititem.html
│       ├── index.html
│       ├── home.html
...
| Layer          | Technology                          |
|----------------|--------------------------------------|
| Backend        | Python, Django, Django REST Framework (in progress) |
| Database       | SQLite (default, swappable via Django ORM) |
| Frontend       | HTML5, Bootstrap 5, vanilla JavaScript (Fetch API) |
| Environment    | Python virtual environment (`venv`), dependencies pinned in `requirements.txt` |
| Architecture   | Server-rendered templates + JSON endpoints for dynamic UI updates (no full page reloads for ordering/billing) |
...
### 🍽️ Menu Management
- Categories (e.g. Beverages, Snacks, Desserts)
- Items grouped by category
- Each item supports either a **single default price** or **Small/Large variant pricing** — never both at once (enforced at the model level)
- Edit/deactivate items and categories without deleting historical order data
...
```bash
# 1. Clone the repository
git clone https://github.com/<your-username>/YourPOS.git
cd YourPOS
...
> **Note:** Always activate the virtual environment (`venv\Scripts\activate` on Windows, `source venv/bin/activate` on macOS/Linux) before running any `manage.py` or `pip` commands, so you're using the project's isolated dependencies rather than any global Python installation.

---

## 🗺️ Roadmap / Ideas for Later

- **REST API layer** using Django REST Framework — exposing tables, menu items, orders, and bills as JSON endpoints (`/api/...`) alongside the existing template-based UI, to enable future mobile apps or alternate frontends
- Per-item sales breakdown (best/worst sellers) using itemized bill snapshots
- Printable/downloadable bill receipts
- Role-based login (waiter vs. manager/admin views)
- KOT (Kitchen Order Ticket) printing for the kitchen counter
...
This project is currently unlicensed / for personal & portfolio use. Add a license (MIT recommended) if you plan to open it up for contributions.
 -->
